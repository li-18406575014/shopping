...
<div class="doc-content">
    ...
    <table cellspacing="0" class="table table-head-bordered table-bordered">
        <a href="<%=request.getContextPath()%>/CoachController/increase">
            <div class="s-btn">
        ...
        <c:forEach items="${ban}" var="obj">
	        <tr>
	            ...
	            <td>${obj.jljs}</td>
	            <td>${obj.shijian}</td>
	            <td><a href="<%=request.getContextPath() %>/CoachController/updateCoach?id=${obj.id}">编辑</a> <a href="<%=request.getContextPath() %>/CoachController/delectCoach?id=${obj.id}">删除</a></td>
	        </tr>
        </c:forEach>
        </tbody>
    </table>
