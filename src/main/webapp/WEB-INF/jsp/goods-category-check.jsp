...
<div class="span95">
    <table cellspacing="0" class="table table-head-bordered table-bordered">
        <a href="<%=request.getContextPath()%>/FourController/increase">
            <div class="s-btn">
        ...
        <c:forEach items="${ban}" var="obj">
	        <tr>
	            ...
	            <td>${obj.fPurpose}</td>
	            <td>${obj.fPurposeJieshao}</td>
	            <td><a href="<%=request.getContextPath() %>/FourController/updatePurpose?id=${obj.id}&type=update">编辑</a> <a href="<%=request.getContextPath() %>/FourController/delectPurpose?id=${obj.id}&type=delete">删除</a></td>
	        </tr>
        </c:forEach>
        </tbody>
    </table>
...
//跳转路径再次查询分页数据
window.location.href="/ajax/F_ObjectiveAddOperation?currentPage="+i+"&pageSize="+pageSize+" "
}
</script>
</html>