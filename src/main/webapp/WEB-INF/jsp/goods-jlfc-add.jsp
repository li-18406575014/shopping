...
<div class="box">
    ...
    <form action="<%=request.getContextPath() %>/CoachController/updateCoachs"  method="post" id="form_2">
    <input type="text" id="file"  value="上传一个文件">
    <input type="text" id="ban"  value="${shiTi.imgname }" name="imgname">
    ...
    <div class="row">
        <div class="control-group">
            ...
            <input type="text"  name="jlname" class="control-text" value="${shiTi.jlname }">
        </div>
    </div>
    <div class="row">
        <div class="control-group">
            <label class="control-label">教练介绍：</label>
            <input type="text"  name="jljs" class="control-text" value="${shiTi.jljs }">
        </div>
    </div>
    <div class="row">
        <div class="control-group">
            <label class="control-label">加入时间：</label>
            <input type="text"  name="jrsj" class="control-text" value="${shiTi.shijian }">
        </div>
    </div>
    <div class="span8">
...
<script>
    $(document).ready(function(){
    	 $('#submit').eq(0).click(function(){
                 $.ajax({
                    url:'<%= request.getContextPath()%>/CoachController/insertCoach',
                    type:"POST",
                    ...
                    success: function(data) {
                        if(data.message=="成功"){
                            ...
                            window.location.href="<%= request.getContextPath()%>/CoachController/selectCoach";
                        }else{
        ...
        $('.jtt').get(0).onchange=function (e){
           var that=this;
            ...
            $.ajax({
               url : '<%= request.getContextPath()%>/CoachController/preview',
               type : "POST",
               data : formData,
               dataType : "json",
